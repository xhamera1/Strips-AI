...
class BeliefNetwork(GraphicalModel):
    ...
    def show(self, fontsize=10, facecolor='orange'):
        ...
        for var in self.variables:
                x,y = var.position
                ax.text(x,y,var.name,bbox=bbox,ha='center', va='center', fontsize=fontsize)
...
class InferenceMethod(Displayable):
    ...
    def show_post(self, obs={}, num_format="{:.3f}", fontsize=10, facecolor='orange'):
        ...
        fig, ax = plt.subplots()
        ax.set_axis_off()
        ax.set_title(self.gm.title+" observed: "+str(obs), fontsize=fontsize)
        self.show_post_graph(fig, ax, obs=obs, num_format=num_format, fontsize=fontsize, facecolor=facecolor)

    def show_post_graph(self, fig, ax, obs={}, num_format="{:.3f}", fontsize=10, facecolor='orange'):
        bbox = dict(boxstyle="round4,pad=1.0,rounding_size=0.5", facecolor=facecolor)
        vartext = {} # variable:text dictionary
        self.nodes = {} # node:variable dictionary (used in probGUI)
        ...
        for var in self.gm.variables:
            x,y = var.position
            node = ax.text(x,y,vartext[var], bbox=bbox, ha='center', va='center', fontsize=fontsize)
            self.nodes[node] = var
